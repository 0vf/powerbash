...
pbash_modcfg() {
	...
	case $1 in
		...
		theme)
			sed -i '' "s/PROMPT_COMMAND=\"[^\"]*\"/PROMPT_COMMAND=\"$2\"/" "$__pbash_configuration_dir/config.cfg"
			printf "${__pbash_command_prefix} PROMPT_COMMAND changed to $2 successfully.\e[m\n";;
		remove)
			sed -i '' "s/__pbash_$2=\"[^\"]*\"//" "$__pbash_configuration_dir/config.cfg"
		...
		*)
			if grep -q "$__pbash_$1=" "$__pbash_configuration_dir/config.cfg"; then
				sed -i '' "s/__pbash_$1=\"[^\"]*\"/__pbash_$1=\"$2\"/" "$__pbash_configuration_dir/config.cfg"
